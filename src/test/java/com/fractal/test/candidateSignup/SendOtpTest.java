...
public class SendOtpTest {
    ...
    public void sendOtpWithEmptyMobileNumber(){
        ...
        Assert.assertEquals(response.jsonPath().getString("message"),"Please Provide Mobile Number");
    }
    ...
    public void sendOtpWithInvalidMobileNumber(){
        ...
        response.then().body("status", equalTo("BAD_REQUEST"));
        Assert.assertNull(response.jsonPath().get("data"));
        Assert.assertEquals(response.jsonPath().getString("message"),"Mobile Number is not valid. Please provide valid mobile number");
    }
...
//    @Test(priority = 4)
    public void sendOtpTest(){
        ...
        Assert.assertEquals(response.jsonPath().getString("message"),"We sent OTP verification code to your mobile number.Please check once...");
    }
}
